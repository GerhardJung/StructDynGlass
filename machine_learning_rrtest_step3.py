...
for m in mode_array:
	for d in dyn_array:
		for t in range(NT-3, NT):
			...
			KA2D_labels_full = pd.read_csv("../../../evaluatation/KA_model/eval_ml_T0.44_bapst2/struct_filion_labels_type0.csv",nrows=(NSTrain+NSTtest)*NLOC, skiprows=[i for i in range(1,NSTrain*NLOC+1)])
			KA2D_features_phys = pd.read_csv("../../../evaluatation/KA_model/eval_ml_T0.44_bapst2/struct_filion_phys_type0.csv",nrows=(NSTrain+NSTtest)*NLOC, usecols=names_list_phys, skiprows=[i for i in range(1,NSTrain*NLOC+1)])
			#print(KA2D_features_phys)
			KA2D_features_thermal = pd.read_csv("../../../evaluatation/KA_model/eval_ml_T0.44_bapst2/struct_filion_thermal_type0.csv",nrows=(NSTrain+NSTtest)*NLOC, usecols=names_list_cg, skiprows=[i for i in range(1,NSTrain*NLOC+1)])
